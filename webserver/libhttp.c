...
// parsing a request, most importantly, get path and method
struct http_request *http_request_parse(int fd)
{
  ...
  do {
    ...
    request->method = malloc(read_size + 1); // store method of the request - GET for now
    ...
    read_size = read_end - read_start;
    if (read_size == 0) break;
    request->path = malloc(read_size + 1); // store the path into object - can be a file or directory
...
// get a response message in case of each status - do I have to make the status myself and call these strings?
...
/* Call http_start_response(fd, status_code) HTTP/1.0 200 OK\r\n*/
/* Call http_send_header(fd, status_code) - Content-Type: text/html\r\n */
/* Call this on http_get_mine_type(filename) -> get value (text/html or text/plain)*/
/* Call http_end_header(fd) - \r\n */
/* Either http_send_string or http_send_data */

// print each response one by one line?
